"""Operational T-measurability test for every GoCo ordering.
For one split, scramble the TruePath/Direct labels of every non-ranking-fold gene (and the truth dictionary of pool
genes) and check that the ordering statistic u_i of affected POOL genes is unchanged. The label-using oracle must change.
...
orders = ['knap', 'source', 'exp0', 'exp1', 'score', 'random1', 'learned', 'learnedgb', 'oracle']
def compute():
    out = {}
    for (j,) in blocks:
...
for k in before:
    ...
    if k[1] == 'oracle': print(f'block {k[0]} {k[1]:10s} changes under scrambling (expected): {not same}'); ok &= (not same)
    else: print(f'block {k[0]} {k[1]:10s} identical under scrambling: {same} (max |diff| = {np.max(np.abs(before[k][0] - after[k][0])):.2e})'); ok &= same
