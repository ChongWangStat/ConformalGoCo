...
lp = fetch(LINKS, OUT + "/links.txt.gz")
ip = fetch(INFO, OUT + "/info.txt.gz")
...
print("[5] RWR top-%d neighbourhoods, focal excluded" % TOPK, flush=True)
r, c = e.a.map(uidx).to_numpy(), e.b.map(uidx).to_numpy()
A = sparse.coo_matrix((np.ones(len(e)), (r, c)), shape=(n, n))
A = (A + A.T).tocsr()
deg = np.asarray(A.sum(axis=0)).ravel()
W = (A @ sparse.diags(1.0 / np.maximum(deg, 1e-12))).tocsr()          # column-stochastic
assert abs(np.asarray(W.sum(axis=0)).ravel()[deg > 0] - 1.0).max() < 1e-9, "W not column-stochastic"
nbr_i, nbr_j, nbr_rank = [], [], []
B = 512
for s in range(0, n, B):
    cols = np.arange(s, min(s + B, n))
    E = np.zeros((n, len(cols))); E[cols, np.arange(len(cols))] = 1.0
    X = E.copy()
    for _ in range(N_ITER):
        X = (1 - RESTART) * (W @ X) + RESTART * E
    X[cols, np.arange(len(cols))] = -np.inf                          # target exclusion
    k = min(TOPK, n - 1)
    idx = np.argpartition(-X, k, axis=0)[:k]
    for t, g in enumerate(cols):
        v = X[idx[:, t], t]
        o = idx[np.argsort(-v), t]
        keep = o[np.isfinite(X[o, t]) & (X[o, t] > 0)]
        nbr_i += [g] * len(keep); nbr_j += list(keep); nbr_rank += list(range(1, len(keep) + 1))
    if s % 2048 == 0:
        print("    RWR %d/%d  %.0fs" % (s, n, time.time() - t0), flush=True)
assert len(nbr_i) > 0, "RWR produced no neighbours"
nb = pd.DataFrame({"focal": np.array(uni)[nbr_i], "neighbor": np.array(uni)[nbr_j], "rank": nbr_rank})
nb.to_csv(OUT + "/string_top50_neighborhoods.csv.gz", index=False)
print("    neighbourhood rows %d | self-loops %d (must be 0)"
      % (len(nb), int((nb.focal == nb.neighbor).sum())), flush=True)
